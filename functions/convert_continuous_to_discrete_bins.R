convert_continuous_to_discrete_bins <- function(inDF, n.discrete.colors) {
    
    ### subset values not == 0
    subDF <- subset(inDF, value > 0)
    
    ### n.discrete.colors takes out value = 0
    n.discrete.colors.rev <- n.discrete.colors - 2

    ### get the value breaks
    value_brks <- round(quantile(subDF$value, 
                                 probs = seq(0, 1, 
                                             100/n.discrete.colors.rev/100)), 2)
    
    value_brks_new <- c(0.0, value_brks, max(subDF$value))
    
    value_brks <- unique(value_brks_new)
    
    #### create categorical plotting labels for each plotting variables
    inDF$value_cat <- cut(inDF$value, 
                          breaks = value_brks)
